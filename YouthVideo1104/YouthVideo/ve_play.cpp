...
int ve_param_vframe(VideoState *is,AVFrame *pFrame,AVPacket *packet){
	...
	//获取当前记录条数
	int nIndex=dlg->m_pVideoDecode->m_lcDecodeframe.GetItemCount();
	//“行”数据结构
	LV_ITEM lvitem;
	...
	//插入表格------------------------
	dlg->m_pVideoDecode->m_lcDecodeframe.InsertItem(&lvitem);
	dlg->m_pVideoDecode->m_lcDecodeframe.SetItemText(nIndex,1,pict_type);
	dlg->m_pVideoDecode->m_lcDecodeframe.SetItemText(nIndex,2,key_frame);
	dlg->m_pVideoDecode->m_lcDecodeframe.SetItemText(nIndex,3,codednum);
	dlg->m_pVideoDecode->m_lcDecodeframe.SetItemText(nIndex,4,pts);
	dlg->m_pVideoDecode->m_lcDecodeframe.SendMessage(WM_VSCROLL, SB_BOTTOM, NULL);
	//准备画！
	//视频帧解码单帧分析-----------------
#if 0
...
int ve_param_aframe(VideoState *is,AVFrame *pFrame,AVPacket *packet){
	...
	//获取当前记录条数
	int nIndex=dlg->m_pAudioDecode->m_lcAudioDecode.GetItemCount();
	//“行”数据结构
	LV_ITEM lvitem;
	lvitem.mask=LVIF_TEXT;
	lvitem.iItem=nIndex;
	...
	//---------------
	dlg->m_pAudioDecode->m_lcAudioDecode.InsertItem(&lvitem);
	dlg->m_pAudioDecode->m_lcAudioDecode.SetItemText(nIndex,1,packet_size);
	dlg->m_pAudioDecode->m_lcAudioDecode.SetItemText(nIndex,2,pts);
	dlg->m_pAudioDecode->m_lcAudioDecode.SetItemText(nIndex,3,dts);
	dlg->m_pAudioDecode->m_lcAudioDecode.SendMessage(WM_VSCROLL, SB_BOTTOM, NULL);
...
static void packet_queue_start(PacketQueue *q)
{
	SDL_LockMutex(q->mutex);
	q->abort_request = 0;
	packet_queue_put_private(q, &flush_pkt);  //向队列q中添加一个只包含flush_pkt的packetlist
	SDL_UnlockMutex(q->mutex);
}
	...
	* pts1 is the dts of the pkt / pts of the frame */
	...
	if (vp->bmp) {
		...
		av_picture_copy(&pict, (AVPicture *)src_frame,
			(enum AVPixelFormat)src_frame->format, vp->width, vp->height);
#else
...
static int stream_component_open(VideoState *is, int stream_index)
{
	...
	case AVMEDIA_TYPE_VIDEO:
		...
		packet_queue_start(&is->videoq);  //把flush_pkt存入队列的起始处  2017-1-5
		//视频线程
